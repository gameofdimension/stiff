...
def test_mask(
    ...
    flex_attention = torch.compile(flex_attn, dynamic=False)
...
def test_checkerboard():
    ...
    # UserWarning: flex_attention called without torch.compile()
    # Call flex_attention with the checkerboard score modification
    output = flex_attn(query, key, value, score_mod=checkerboard)

    # Compile and run
    compiled_flex_attention = torch.compile(flex_attn)
